...
class QuranSyncManagerTest {
    ...
    @Test
    fun completeSyncCommitsAllSurahsAndSortsOutOfOrderAyat() = runTest {
    ...
    @Test
    fun atomicFailureLeavesNoPartialDataset() = runTest {
        server.dispatcher = fixtureDispatcher(failAyatForSurah = 50)
        val manager = manager()

        val result = manager.sync(stableVersion = 1)

        assertTrue(result is ApiResult.Failure && result.isRetryable)
        assertEquals(0, database.quranSurahDao().count())
        assertEquals(0, database.quranVerseDao().count())
    }

    @Test
    fun refreshFailurePreservesPreviousCompleteDataset() = runTest {
        server.dispatcher = fixtureDispatcher()
        val manager = manager()
        assertTrue(manager.sync(stableVersion = 1) is ApiResult.Success)
        ...
        assertTrue(refreshResult is ApiResult.Failure && refreshResult.isRetryable)
    ...
    @Test
    fun duplicateRemoteAyatIdAcrossSurahsIsRejectedAndNothingCommitted() = runTest {
    ...
    private fun surahListEnvelope(): QuranEnvelopeDto<List<QuranSurahDto>> = QuranEnvelopeDto(
        code = 200,
        res = "success",
        data =
    ...
    ): QuranEnvelopeDto<List<QuranAyatDto>> = QuranEnvelopeDto(
        code = 200,
        res = "success",
        data =
