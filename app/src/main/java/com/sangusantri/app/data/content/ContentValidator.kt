...
object ContentValidator {
     ...
     */
    fun validateList(response: ContentListResponseDto): Validation {
        val reason =
            validateRemoteSchema(response.schemaVersion) ?: validateListItems(response)
        return Validation.of(reason)
    }
     ...
     */
    fun validateDetail(detail: ContentDetailDto): Validation {
        val reason =
    ...
    fun validateListItem(item: ContentListItemDto): Validation = Validation.of(listItemReason(item))

    private fun listItemReason(item: ContentListItemDto): String? = when {
    ...
    private fun validateDetailIdentifiers(detail: ContentDetailDto): String? = when {
    ...
    fun isAllowedImageUrl(imageUrl: String?): Boolean = imageUrl == null ||
        (
    ...
    private fun validateStep(step: ContentStepDto): String? = when {
