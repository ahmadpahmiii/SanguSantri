...
object NahwuQuizValidator {
    ...
    fun validate(bank: NahwuQuizBankDto): Validation {
        val reason = validateSchema(bank) ?: validatePackages(bank)
        return Validation.of(reason)
    }

    private fun validateSchema(bank: NahwuQuizBankDto): String? = if (bank.schemaVersion != SUPPORTED_SCHEMA_VERSION) {
    ...
    @Suppress("ReturnCount")
    private fun validateQuestion(question: NahwuQuizQuestionDto): String? = when {
