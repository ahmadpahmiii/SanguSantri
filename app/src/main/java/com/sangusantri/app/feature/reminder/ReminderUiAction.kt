...
sealed interface ReminderUiAction {
    ...
    data class SaveReminder(val reminder: Reminder) : ReminderUiAction

    data class ToggleEnabled(val reminder: Reminder) : ReminderUiAction

    /** Confirmed via a dialog in the UI layer — the ViewModel never deletes without confirmation. */
    data class DeleteReminder(val reminderId: String) : ReminderUiAction
}
