...
The HTML/JSON files are local design-reference sources, not payloads exported
from an external design tool. They exist because this design pass is authored
directly in HTML/JSON/PNG rather than a hosted design tool. If these screens
are later redrawn in a dedicated design tool, retain the PNG names and record
the resulting references in `docs/design/DESIGN_HANDOFF.md`.
