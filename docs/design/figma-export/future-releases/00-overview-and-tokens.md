...
superseded by that decision; see the Navigation IA table and Figma output
plan sections below for the corrected model.

## Status

**The Figma MCP connection is rate-limited on the team's Starter plan** (confirmed
2026-07-29 — `get_metadata`/`whoami` both rejected with a tool-call-limit error,
matching the same blocker `docs/design/FIGMA_HANDOFF.md` recorded on 2026-07-26).
No page, frame, component, or node has been created in the live Figma file. This
folder is the complete offline build specification for that work instead —
screens, states, layer names, token bindings, component variants, icon names,
and interaction annotations for every item in scope — written so a future
session with working Figma access can execute it directly as `use_figma`
scripts with no re-discovery needed. See `06-validation-matrix.md` for the
frame ledger (names planned now, real node IDs filled in once built).

Nothing in this pass touches the live Figma file, `docs/PROGRESS.md`, or any
engineering doc — design assets only, per the request.
...
2. The **actual Compose design-system source** —
   ...
   against the exported Figma node data, not assumed.
...
4. `docs/design/DESIGN_SYSTEM.md`, `docs/design/ACCESSIBILITY.md`,
   `docs/design/FIGMA_HANDOFF.md`, `docs/reviews/figma-product-alignment.md`.
...
`fills`/`boundVariables` of all 12 exported Figma frames (script: walk every
...
| Figma `VariableID` | Hex       | Compose token (`Color.kt`)                 | Material 3 role                            | Seen on (examples)                                      |
|--------------------|-----------|--------------------------------------------|--------------------------------------------|---------------------------------------------------------|
...
Dark-mode values: no dark-mode Figma frame has ever been exported (same gap
...
## Figma output plan (for when Figma access returns)

* **New page**: `Future Releases — 0.0.2 to 0.0.5` (renamed from `0.4.0` —
  Nahwu Quiz moved to `0.0.5`, ADR 0013).
* **Sections** (Figma `SectionNode`, left-to-right): `0.0.2 — Tasbih` ·
...
* **Every frame**: Auto Layout throughout (no absolute positioning except
  ...
  Figma comment-pin describing interaction/haptic/accessibility behavior
  that a static frame can't show.
* **Components page**: one dedicated sub-area per component with its full
  variant grid, following `figma-generate-library`'s one-page-per-component-
  family default, componentized (not flat one-off frames) per that skill's
  Phase 3 rules.

## Execution path once Figma is reachable
...
3. Create the page and sections (one `use_figma` call).
...
5. Build each release frame-by-frame per its own spec file, one section per
   `use_figma` call, `get_screenshot` validating after each — per
   `figma-generate-design`'s Step 4 workflow.
