...
Figma component **set** with named variant properties (not a flat frame per
state), built with Auto Layout, ready for `combineAsVariants` per
`figma-generate-library`. Minimum touch target is 48×48dp on every
