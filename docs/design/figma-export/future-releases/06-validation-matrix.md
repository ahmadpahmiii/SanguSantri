...
dark hex invented, since no dark-mode Figma frame has ever been exported):
...
single Figma variable mode-swap (Light/Dark) produces every dark frame,
...
* Build **one explicit RTL frame per release** as a spot-check (same set as
  the dark-mode spot-checks above), each mirrored via Figma's own
...
session with working Figma access actually creates it — this table is the
checklist that session fills in, not a claim that these nodes exist yet.
