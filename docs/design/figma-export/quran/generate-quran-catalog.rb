...
CSS = <<~CSS.freeze
  ...
  *{box-sizing:border-box}html,body{width:360px;height:800px;margin:0;overflow:hidden;background:transparent;font-family:Inter,Roboto,Arial,sans-serif}.screen{position:relative;width:360px;height:800px;overflow:hidden;border-radius:28px;color:var(--text);background:var(--bg)}
  ...
  .tabs-viewport{height:52px;overflow:hidden;border-bottom:1px solid var(--outline)}.tabs{width:490px;height:52px;display:flex}.tabs.last{transform:translateX(-130px)}.tab{min-width:110px;height:52px;display:grid;place-items:center;border-bottom:2px solid transparent;color:var(--muted);font-size:14px}.tab.wide{min-width:150px}.tab.active{color:var(--primary);border-color:var(--primary)}
...
def tabs(active)
  names = [["surah", "Surah", ""], ["juz", "Juz", ""], ["bookmark", "Bookmark", ""], ["recent", "Terakhir Dibaca", "wide"]]
  last = active == "recent" ? " last" : ""
  inner = names.map { |id, label, wide| "<div class=\"tab #{wide} #{active == id ? 'active' : ''}\">#{label}</div>" }.join
  "<div class=\"tabs-viewport\"><div class=\"tabs#{last}\">#{inner}</div></div>"
end

def hub(content, active:, search: nil, refresh: nil)
  ...
  top_bar("Al-Qur'an", "Al-Qur'an Kemenag", plain: true) +
    "<div class=\"hub-body\">#{continue_card(refresh: refresh)}#{tabs(active)}#{search_html}#{content}</div>"
end
...
def settings_screen
  <<~HTML
    ...
    <div class="settings">
      ...
      <div class="control"><div class="control-head"><span>Ukuran Arab</span><strong>34 sp</strong></div><div class="slider"><span style="width:58%"></span></div></div>
      <div class="control"><div class="control-head"><span>Jarak baris Arab</span><strong>1,8×</strong></div><div class="slider"><span style="width:68%"></span></div></div>
...
def state_meta(screen, state, source_data: "No Quran payload rendered")
  {
    ...
    figma: { page: "03 Al-Qur'an Kemenag", nodeId: nil },
  }
end
...
add.call("04a-quran-hub-recent-populated", "Beranda Al-Qur'an — Terakhir Dibaca", hub("<div class=\"list\">#{position_rows(:recent)}</div>", active: "recent"), { tab: "Terakhir Dibaca", content: "populated" }, "Positions use supplied Al-Fajr and An-Nas API metadata; timestamps are presentation fixtures")
add.call("04b-quran-hub-recent-empty", "Beranda Al-Qur'an — Riwayat kosong", hub('<div class="empty"><div class="empty-mark">◷</div><h2>Belum ada aktivitas membaca</h2><p>Riwayat akan muncul setelah posisi bacaan berpindah minimal satu ayat.</p></div>', active: "recent"), { tab: "Terakhir Dibaca", content: "empty" })
